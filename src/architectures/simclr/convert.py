...
def convert(tf_path, pth_path):
    # 1. read tensorflow weight into a python dict
    vars_list = tf.train.list_variables(tf_path)
    ...
    for i_conv in range(len(conv_keys)):
        ...
        assert (
            torch.from_numpy(sd[conv_keys[i_conv]]).permute(3, 2, 0, 1).shape
            == m.weight.data.shape
        )
    ...
    for i_bn in range(len(bn_keys)):
        ...
        m.running_mean = torch.from_numpy(
            sd["base_model/" + bn_keys[i_bn] + "/moving_mean"]
        )
        m.running_var = torch.from_numpy(
            sd["base_model/" + bn_keys[i_bn] + "/moving_variance"]
        )

    model.linear_output.weight.data = torch.from_numpy(
        sd["head_supervised/linear_layer/dense/kernel"]
    ).t()
    model.linear_output.weight.bias = torch.from_numpy(
        sd["head_supervised/linear_layer/dense/bias"]
    )
